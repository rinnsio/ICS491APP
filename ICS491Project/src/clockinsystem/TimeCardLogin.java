...
public class TimeCardLogin {
  ...
  private java.sql.Timestamp getTimeIn() {
      try {
          ...
          if (results.isBeforeFirst()) {
              results.first();
              // add date and time together and return the number format of it.
              java.sql.Timestamp datee = new java.sql.Timestamp(results.getDate(1).getTime());
                ...
                System.out.println(results.getDate(1)+ " "+ results.getTime(1));
                SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                // format it to a timestamp object 
		Date dateformat = formatter.parse(datetime);
                java.sql.Timestamp finaldatatime = new java.sql.Timestamp(dateformat.getTime());
  ...
  public void clockout() {
      ...
      try{
            ...
            Double hours   =  (new Long((milliseconds / (1000*60*60)) % 24)).doubleValue();
